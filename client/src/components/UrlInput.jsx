...
export default function UrlInput({ onVideoFound, onFileUpload }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'Failed to fetch video info. Try uploading the video instead.');
            console.error(err);
        } finally {
            setLoading(false);
        }
            ...
            <div className={`card pulse-glow transition-all duration-1000 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="relative group">
                        <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Paste YouTube link here..." className="input-field pl-14 text-base py-4 rounded-2xl" disabled={loading} />
                        ...
                        {url && !loading && (
                            <button type="button" onClick={() => { setUrl(''); setError(''); }} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 text-sm transition-colors">Clear</button>
                        )}
                    </div>
                    ...
                    <div
                        ...
                        onDrop={(e) => { e.preventDefault(); setDragActive(false); const file = e.dataTransfer.files[0]; if (file && file.type.startsWith('video/')) { onFileUpload(file, forceShorts); } else { setError('Please upload a video file'); } }}
                        onClick={() => fileInputRef.current?.click()}
                        className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-all duration-300 ${dragActive ? 'border-indigo-500 bg-indigo-500/10' : 'border-slate-700 hover:border-slate-600 bg-slate-800/30'}`}
                    >
                        <input ref={fileInputRef} type="file" accept="video/*" className="hidden" onChange={(e) => { const file = e.target.files[0]; if (file) onFileUpload(file, forceShorts); }} />
                        <Upload className="w-6 h-6 mx-auto mb-2 text-slate-500" />
                        <p className="text-slate-400 text-sm"><span className="text-indigo-400 font-medium">Click to upload</span> or drag and drop</p>
                    ...
                    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-indigo-500/5 to-purple-500/5 border border-indigo-500/10 rounded-2xl">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-xl text-indigo-400"><Zap className="w-4 h-4" /></div>
                    ...
                    {error && (
                        <div className="text-red-400 text-sm bg-red-500/10 p-4 rounded-2xl border border-red-500/20 animate-slide-up">
                            <div className="flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-red-400 flex-shrink-0" /><span>{error}</span></div>
                        </div>
                    )}

                    <button type="submit" disabled={loading || !url} className="btn-primary w-full flex items-center justify-center gap-3 text-base py-4 rounded-2xl">
                        {loading ? (<><Loader2 className="w-5 h-5 animate-spin" /><span>Analyzing Video...</span></>) : (<><span>Analyze Video</span><ArrowRight className="w-5 h-5" /></>)}
                    </button>
                </form>
            </div>
